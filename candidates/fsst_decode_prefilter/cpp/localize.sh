...
# candidate's cwida FSST copy cannot clash with the `fsst` candidate's cwida
# copy at the final harness link — WITHOUT touching weak COMDAT symbols. Same
...
nm --defined-only "$obj" \
  | awk '($2=="T" || $2=="D" || $2=="B") \
         && $3 != "lb_candidate_fsst_decode_prefilter" \
         && $3 != "lb_candidate_dict_fsst_decode_prefilter" { print $3 }' \
  | sort -u > "$list"
