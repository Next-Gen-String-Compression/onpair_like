//! Glue crate for the C++ `fsst_like_tum` candidate: build.rs compiles cpp/ via
...
extern "C" {
    fn lb_candidate_fsst_like_tum() -> *const LbCandidate;
}

pub fn vtable() -> &'static LbCandidate {
    // The C++ side returns a pointer to a static vtable.
    unsafe { &*lb_candidate_fsst_like_tum() }
}
