...
public:
    ...
    size_t bytes_used() const noexcept {
        return edge_labels_.size()   * sizeof(uint8_t)
             + edge_targets_.size()  * sizeof(State)
             + child_offsets_.size() * sizeof(uint16_t)
             + fail_.size()          * sizeof(State)
             + (is_accepting_.size() + 7) / 8;  // std::vector<bool> is bitpacked
    }
...
inline AhoCorasickTrie::AhoCorasickTrie(std::span<const std::string_view> patterns) {
    ...
    // -- 0. Optional Pruning Setup ------------------------------------------
    const bool prune_prefixes = true;
    std::vector<std::string_view> sorted_patterns;
    std::span<const std::string_view> active_patterns = patterns;

    if (prune_prefixes) {
        sorted_patterns.assign(patterns.begin(), patterns.end());
        // Sort by length ascending: ensures prefixes are inserted before supersets
        std::sort(sorted_patterns.begin(), sorted_patterns.end(),
                  [](std::string_view a, std::string_view b) {
                      return a.size() < b.size();
                  });
        active_patterns = sorted_patterns;
    }

    // -- 1. Build Trie (with inline sorted insertion) -----------------------
    for (const auto& pat : active_patterns) {
        if (pat.empty()) {
            is_accepting_[ROOT_STATE] = true;
            if (prune_prefixes) break;
            continue;
        }
        ...
        for (size_t i = 0; i < pat.size(); ++i) {
            ...
            if (prune_prefixes && is_accepting_[cur]) {
                // Current pattern is a duplicate or has an existing prefix; skip it
                break;
            }
        }
